...
console.log("Initial Goals from Store:", store.teamMemberData);
const goals = ref([]);
const goals_ = ref([])
...
const fetchData = () => {
  if (!store.teamMemberData.length) {
    console.warn('No team member data available.');
    return;
  }
  
  goals.value = store.teamMemberData.filter(goal => goal.recordOwner === selectedOwner.value);
  console.log('Filtered Team Member Data:', goals.value);
};

// Watch for changes to the route parameter
watch(
  () => route.params.owner,
  (newOwner) => {
...
watch([() => props.year, () => props.quarter, selectedOwner], fetchFilteredGoals);

onMounted(async () => {
    if (!store.teamMemberData.length) {
    store.fetchTeamMemberData().then(() => {
      fetchData();
    });
  } else {
    fetchData();
...
//   console.log('Goals before filtering:', goals.value);

//   const filtered = goals.value.length > 0 ? goals.value.filter(item => {
//     console.log('Filtering - PlanTypeId:', item.planTypeId, 'PlanTermId:', item.planTermId);
...
// });
const filteredGoals = computed(() => {
  const filtered = goals.value.filter(item => 
    item.planTypeId === parseInt(selectedOption.value) && 
    item.planTermId === parseInt(selectedTerm.value)
  );
  console.log('Filtered Goals:', filtered);
  return filtered;
});
