...
onMounted(async () => {
  ...
  taskDetails.value = await store.updateTask(taskId.value);
});
...
<template>
  <div class="table-responsive">
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="item in tasks" @dblclick="$router.push({name: 'Tasks Details', params: {id: item.deliverableId}})">
          <td scope="row">{{ item.deliverableTypeId }}</td>
