...
const fetchFilteredGoals = async () => {
  await store.fetchGoals();
  goals.value = store.goals.filter(goal => {
    const goalDate = new Date(goal.targetCompletionDate); 
    const goalYear = goalDate.getFullYear();
    const goalQuarter = Math.ceil((goalDate.getMonth() + 1) / 3); 
    return goalYear === props.year && goalQuarter === props.quarter;
  });
};

// Watch for changes in year and quarter
watch([() => props.year, () => props.quarter], fetchFilteredGoals);
