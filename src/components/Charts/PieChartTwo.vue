...
const countStatuses = () => {
  const filteredPlans = allDevelopmentPlans.value.filter(plan => plan.planTypeId === 1); // Filter by planTypeId for career goals and aspirations
...
const updateChartData = () => {
  ...
  const filteredPlans = allDevelopmentPlans.value.filter(plan => plan.planType && plan.planType.id === 1);
  console.log("Filtered plans (planType.id === 1):", filteredPlans);
...
if (total > 0) {
  ...
  console.log("No plans with planType.id === 1 found.");
}
  ...
  // }
  if (total > 0) {
  chartInstance.data.datasets[0].data = [statusCounts[1], statusCounts[2], statusCounts[3]];
  chartInstance.update();
}
};
